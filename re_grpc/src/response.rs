...
impl TTimestamp {
    ...
    /// Convert TTimestamp to protobuf Timestamp
    pub fn to_grpc(&self) -> ::prost_types::Timestamp {
    ...
    /// Convert protobuf Timestamp to TTimestamp
    pub fn from_grpc(ts: Option<::prost_types::Timestamp>) -> Self {
        match ts {
...
impl TStatus {
    /// Create a successful TStatus with OK code
    pub fn ok() -> Self {
        TStatus {
            code: TCode::OK,
            message: String::new(),
    ...
    /// Convert grpc Status to Result, returning error if not OK
    pub fn from_grpc_result(status: re_grpc_proto::google::rpc::Status) -> Result<(), crate::error::REClientError> {
        if status.code == 0 {
            return Ok(());
        }
        Err(crate::error::REClientError {
            code: TCode(status.code),
            message: status.message,
            group: crate::error::TCodeReasonGroup::UNKNOWN,
        })
    }
}
