...
impl REClient {
    ...
    pub async fn download_blob(
        ...
        let tdigest = TDigest::from_grpc(digest.clone());
        let response = self.download(
            RemoteExecutionMetadata::default(),
            DownloadRequest {
                inlined_digests: Some(vec![tdigest.clone()]),
                file_digests: None,
                ..Default::default()
            },
        ).await
        ...
        response.inlined_blobs
            .and_then(|blobs| {
                blobs.into_iter()
                    .find(|b| b.digest.hash == digest.hash)
                    .map(|b| b.blob)
            })
            .ok_or_else(|| anyhow::anyhow!("Blob not found in response"))
    }

    pub async fn upload_blob(
        &self,
        data: Vec<u8>,
    ) -> anyhow::Result<re_grpc_proto::build::bazel::remote::execution::v2::Digest> {
        let digest = crate::digest::compute_digest(&data);
        
        self.upload_blob_with_digest(
            data,
            digest.clone(),
            RemoteExecutionMetadata::default(),
        ).await?;
    ...
    pub async fn upload_directory_tree_from_path(
        &self,
        path: &std::path::Path,
    ) -> anyhow::Result<re_grpc_proto::build::bazel::remote::execution::v2::Digest> {
        use crate::action::directory::DirectoryBuilder;
        
        let builder = DirectoryBuilder::from_directory(path).await
            .context("Failed to build directory tree")?;
        
        let root_digest = builder.root_digest()
            .ok_or_else(|| anyhow::anyhow!("No root directory generated"))?
            .clone();
        
        let files = builder.files_to_upload().to_vec();
        let directories = builder.directories_to_upload();
        
        self.upload_directory_tree(directories, files, RemoteExecutionMetadata::default())
            .await?;
        
        Ok(root_digest.to_grpc())
    }

    pub fn download_directory_tree<'a>(
        &'a self,
        digest: &'a re_grpc_proto::build::bazel::remote::execution::v2::Digest,
        dest: &'a std::path::Path,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = anyhow::Result<()>> + Send + 'a>> {
        Box::pin(async move {
            use prost::Message;
            use re_grpc_proto::build::bazel::remote::execution::v2::Directory;
            
            let directory_blob = self.download_blob(digest).await
                .context("Failed to download root directory")?;
            let directory = Directory::decode(&directory_blob[..])
                .context("Failed to decode Directory")?;
            
            tokio::fs::create_dir_all(dest).await
                .context("Failed to create destination directory")?;
            
            for file_node in &directory.files {
                let file_digest = file_node.digest.as_ref()
                    .ok_or_else(|| anyhow::anyhow!("Missing file digest"))?;
                let file_data = self.download_blob(file_digest).await
                    .with_context(|| format!("Failed to download file: {}", file_node.name))?;
                
                let file_path = dest.join(&file_node.name);
                tokio::fs::write(&file_path, file_data).await
                    .with_context(|| format!("Failed to write file: {}", file_node.name))?;
            ...
            for dir_node in &directory.directories {
                let dir_digest = dir_node.digest.as_ref()
                    .ok_or_else(|| anyhow::anyhow!("Missing directory digest"))?;
                let dir_path = dest.join(&dir_node.name);
                self.download_directory_tree(dir_digest, &dir_path).await
                    .with_context(|| format!("Failed to download directory: {}", dir_node.name))?;
            }
