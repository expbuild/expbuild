...
use re_server::{
    ...
    cas::CasManager,
    config::ServerConfig,
    execution::{ExecutionManager, WorkerScheduler},
    grpc::{
...
use re_grpc_proto::expbuild::worker::v1::worker_scheduler_server::WorkerSchedulerServer;
...
async fn main() -> Result<()> {
    ...
    tracing::info!("Initializing remote worker scheduler...");
    let worker_scheduler = WorkerScheduler::new(Default::default());
    worker_scheduler.clone().start_maintenance_loop().await;
    
    let execution_manager = Arc::new(ExecutionManager::with_worker_scheduler(
        cas_manager.clone(),
        cache_manager.clone(),
    ...
    tracing::info!("Enabling Worker Scheduler API for remote workers");
    let worker_scheduler_service = WorkerSchedulerService::new(worker_scheduler);
    server_builder = server_builder.add_service(WorkerSchedulerServer::new(worker_scheduler_service));
